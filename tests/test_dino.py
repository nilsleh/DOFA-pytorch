...
#         params=classification_configs,
#     )
#     def model(self, model_config, other_args, data_config, monkeypatch: MonkeyPatch, tmp_path: Path):
#         # mock weights
...
#             groups = ['s1_encoder', 's1_GAP_FFN', 's2_encoder', 's2_GAP_FFN', 'joint_encoder']
#             new_dict = {}
#             for group in groups:
#                 new_dict[group] = {}
...
#         # update url
#         # monkeypatch.setattr(model_class, 'url', os.path.join(tmp_path, '{}'))
...
#         if 'croma' in model_name:
#             with pytest.raises(RuntimeError, match=re.escape('Error(s) in loading state_dict for Sequential:\n\tMissing key(s) in state_dict: "3.weight", "3.bias". ')):
