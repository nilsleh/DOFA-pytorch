...
class RCF(nn.Module):
    ...
    def forward(self, x: Tensor) -> list[Tensor]:
        ...
        for scale in range(self.num_scales):
            ...
            # Pool and squeeze
            x1a = (
                F.adaptive_avg_pool2d(x1a, (self.spatial_dim, self.spatial_dim))
                .squeeze(-1)
                .squeeze(-1)
            )
            x1b = (
                F.adaptive_avg_pool2d(x1b, (self.spatial_dim, self.spatial_dim))
                .squeeze(-1)
                .squeeze(-1)
            )
    ...
    def __init__(self, args, config, data_config):
        ...
        if config.num_scales == 1:
            self.neck = Feature2Pyramid(
                embed_dim=kwargs["features"], rescales=[4, 2, 1, 0.5]
            )
