...
class DummyDataset(Dataset):
    ...
    def __init__(self, img_size: int, num_channels: int, num_classes: int, split: str, transforms: Any, num_samples: int = 2) -> None:
        super().__init__()
        self.split = split
...
class DummyWrapper:
    ...
    def create_dataset(self) -> Tuple[Dataset, Dataset, Dataset]:
        ...
        dataset_train = DummyDataset(split="train", transforms=train_transform, img_size=self.img_size, num_samples=2, num_classes=self.num_classes, num_channels=self.num_channels)
        dataset_val = DummyDataset(split="val", transforms=eval_transform, img_size=self.img_size, num_samples=1, num_classes=self.num_classes, num_channels=self.num_channels) 
        dataset_test = DummyDataset(split="test", transforms=eval_transform, img_size=self.img_size,num_samples=1, num_classes=self.num_classes, num_channels=self.num_channels)
